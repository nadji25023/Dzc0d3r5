...
class AdminInit {
    ...
    function styles()     {
        ...
        wp_enqueue_style('radio-image', THEME_CSS_LIBRARIES . '/radio-image-select.min.css', array(), '1.0', 'screen');
        wp_enqueue_style('jquery-ui', THEME_CSS_LIBRARIES . '/smoothness/jquery-ui-1.9.2.custom.min.css', array(), '1.9.2', 'screen');

        $my_style = new CssGenerator();
        $my_style->get_admin_css();
    ...
    function scripts() {
        ...
        wp_enqueue_script('jquery-radioImage', THEME_JS_LIBRARIES . "/jquery.radioImageSelect.min.js", array('jquery'), '1.0', false);
