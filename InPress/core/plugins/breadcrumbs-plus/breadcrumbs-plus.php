...
function breadcrumbs_plus( $args = '' ) {
	...
	$defaults = array(
		...
		'suffix' => '</p>',
		'title' => __( 'You are here: ', 'inpress_front' ),
	...
	elseif ( is_singular() ) {
		...
		elseif ( 'portfolio' === $wp_query->post->post_type ) {
			...
			if(!empty($parent_page)){
				$parents = breadcrumbs_plus_get_parents( $parent_page, $separator );
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_date() ) {
			...
			if ( is_day() )
				$html .= _bold_( sprintf(__('Daily Archive for: ‘%s’','striking_front'),get_the_time('F jS, Y')), $args['bold'] );
			
			elseif ( is_month() )
				$html .= _bold_( sprintf(__('Monthly Archive for: ‘%s’','striking_front'),get_the_time('F, Y')), $args['bold'] );
					
			elseif ( is_year() )
				$html .= _bold_( sprintf(__('Yearly Archive for: ‘%s’','striking_front'),get_the_time('Y')), $args['bold'] );

		} 
		elseif ( is_author() )
			$html .= _bold_( sprintf(__('Author Archive for: ‘%s’','striking_front') , get_the_author_meta( 'display_name', $wp_query->post->post_author )), $args['bold'] );
	}

	/* If viewing search results. */
	elseif ( is_search() ) 
		$html .= $home . $separator . _bold_( sprintf(__( 'Search Results for: ‘%s’', 'striking_front' ),stripslashes( strip_tags( get_search_query() ) ) ), $args['bold'] );

	/* If viewing a 404 error page. */
	elseif ( is_404() )
		$html .= $home . $separator . _bold_( __( 'Page Not Found', 'striking_front' ), $args['bold'] );
